...
#include "command/p2p_cms_ipcamera_channel_command_common.h"
...
static libpiziot_p2p_clientsdk_channel_client_callback_t m_p2p_cms_ipcamera_callback;

libpiziot_os_type_func_result_e p2p_cms_ipcamera_channel_main_get_net_status(int32_t Achannel_client_handle, int32_t Achannel_id, libpiziot_p2p_common_result_e *Alpnet_startus) {
    ...
    do {
        ...
        if (channel_net_result != (*Alpnet_startus)) {
            ...
            case LIBPIZIOT_P2P_COMMON_RESULT_CONNECTING: {
                TRACEA("PIZCmsCam:CHANNEL HANDLE:(%d),ID:(%d) CONNECTING\n", Achannel_client_handle, Achannel_id);
            }
                                                        break;
            case LIBPIZIOT_P2P_COMMON_RESULT_MODE_LAN_CHANNEL_IN_LAN: {
                TRACEA("PIZCmsCam:CHANNEL HANDLE:(%d),ID:(%d) LAN_CHANNEL_IN_LAN\n", Achannel_client_handle, Achannel_id);
            }
                                                                     break;
            case LIBPIZIOT_P2P_COMMON_RESULT_MODE_LAN_CHANNEL_IN_WAN: {
                TRACEA("PIZCmsCam:CHANNEL HANDLE:(%d),ID:(%d) LAN_CHANNEL_IN_WAN\n", Achannel_client_handle, Achannel_id);
            }
                                                                     break;
            case LIBPIZIOT_P2P_COMMON_RESULT_MODE_WAN_CHANNEL_IN_PTP: {
                TRACEA("PIZCmsCam:CHANNEL HANDLE:(%d),ID:(%d) WAN_CHANNEL_IN_PTP\n", Achannel_client_handle, Achannel_id);
            }
                                                                     break;
            case LIBPIZIOT_P2P_COMMON_RESULT_MODE_WAN_CHANNEL_IN_RELAY: {
                TRACEA("PIZCmsCam:CHANNEL HANDLE:(%d),ID:(%d) WAN_CHANNEL_IN_RELAY\n", Achannel_client_handle, Achannel_id);
            }
                                                                       break;
            default: {
                TRACEA("PIZCmsCam:CHANNEL HANDLE:(%d),ID:(%d) ERROR !! = %d\n", Achannel_client_handle, Achannel_id, channel_net_result);
            }
                     break;
            }
        }
...
static libpiziot_os_type_func_result_e p2p_cms_ipcamera_channel_main_free(p2p_cms_main_ipcamera_thread_info_t *Alpp2p_cms_main_ipcamera_thread_info, int32_t Athread_info_index) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS;
    do {
        p2p_cms_ipcamera_channel_main_thread_info_t *lpthread_info = Alpp2p_cms_main_ipcamera_thread_info->lpp2p_cms_ipcamera_channel_main_thread_info[Athread_info_index];
        ...
        }
        {
            ...
            Alpp2p_cms_main_ipcamera_thread_info->lpp2p_cms_ipcamera_channel_main_thread_info[Athread_info_index] = 0;
        }
    } while (0);
    return rval;
}

static libpiziot_os_type_func_result_e p2p_cms_ipcamera_channel_main_malloc(p2p_cms_main_ipcamera_thread_info_t *Alpp2p_cms_main_ipcamera_thread_info, int32_t Athread_info_index, uint32_t Asend_data_max_size, uint32_t Arecv_data_max_size) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    p2p_cms_ipcamera_channel_main_thread_info_t *lpthread_info = 0;
    do {
        ...
        if (Alpp2p_cms_main_ipcamera_thread_info->lpp2p_cms_ipcamera_channel_main_thread_info[Athread_info_index] != 0) {
...
#else
        {
            ...
            uint64_t check_size = sizeof(p2p_cms_ipcamera_channel_main_thread_info_t);
        ...
        lpthread_info = (p2p_cms_ipcamera_channel_main_thread_info_t *)libpiziot_os_malloc(sizeof(p2p_cms_ipcamera_channel_main_thread_info_t));
        if (lpthread_info == 0) {
            TRACEA("%s:out of memory,error !!\n", LIBPIZIOT_OS__FUNCTION__);
            break;
        }
        Alpp2p_cms_main_ipcamera_thread_info->lpp2p_cms_ipcamera_channel_main_thread_info[Athread_info_index] = lpthread_info;
        {
            memset_addr(p2p_cms_ipcamera_channel_main_thread_info_t, lpthread_info, 0, sizeof(p2p_cms_ipcamera_channel_main_thread_info_t));
    ...
    if (rval != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
        p2p_cms_ipcamera_channel_main_free(Alpp2p_cms_main_ipcamera_thread_info, Athread_info_index);
        TRACEA("%s,error !!\n", LIBPIZIOT_OS__FUNCTION__);
    }
    return rval;
}

static void p2p_cms_ipcamera_channel_main_start_thread_one(p2p_cms_main_ipcamera_thread_info_t *Alpp2p_cms_main_ipcamera_thread_info, int32_t Athread_info_index, libpiziot_os_pthread_routine_p Alppthread_routine) {
    if (Alppthread_routine == 0) return;
    if (Alpp2p_cms_main_ipcamera_thread_info->lpp2p_cms_ipcamera_channel_main_thread_info[Athread_info_index] == 0) return;
    {
        p2p_cms_ipcamera_channel_main_thread_info_t *lpthread_info = Alpp2p_cms_main_ipcamera_thread_info->lpp2p_cms_ipcamera_channel_main_thread_info[Athread_info_index];
        ...
        do {
            ...
            if ((libpiziot_os_pthread_get_status(&(lpthread_info->thread_instance)) == LIBPIZIOT_OS_PTHREAD_STATUS_CLOSED)) {
                if (libpiziot_os_pthread_create(&(lpthread_info->thread_instance), Alppthread_routine, LIBPIZIOT_OS_PTHREAD_STACK_SIZE_AUTO, "core_cms_cam_chc") != 0) {
...
static void p2p_cms_ipcamera_channel_main_stop_thread_one(p2p_cms_main_ipcamera_thread_info_t *Alpp2p_cms_main_ipcamera_thread_info, int32_t Athread_info_index) {
    if (Alpp2p_cms_main_ipcamera_thread_info->lpp2p_cms_ipcamera_channel_main_thread_info[Athread_info_index] == 0) return;
    {
        p2p_cms_ipcamera_channel_main_thread_info_t *lpthread_info = Alpp2p_cms_main_ipcamera_thread_info->lpp2p_cms_ipcamera_channel_main_thread_info[Athread_info_index];
        if (lpthread_info->instance_mutex.init != LIBPIZIOT_OS_MUTEX_STUCT_INIT_CHECK_VALUE) return;
...
void p2p_cms_ipcamera_channel_main_stop_thread_all(p2p_cms_main_ipcamera_thread_info_t *Alpp2p_cms_main_ipcamera_thread_info) {
    {
        uint32_t thread_info_index;
        for (thread_info_index = 0; thread_info_index < P2P_CMS_IPCAMERA_CHANNEL_MAIN_THREAD_MAX; thread_info_index++) {
            p2p_cms_ipcamera_channel_main_stop_thread_one(Alpp2p_cms_main_ipcamera_thread_info, thread_info_index);
        }
    }
    {
        uint32_t thread_info_index;
        for (thread_info_index = 0; thread_info_index < P2P_CMS_IPCAMERA_CHANNEL_MAIN_THREAD_MAX; thread_info_index++) {
            p2p_cms_ipcamera_channel_main_thread_info_t *lpthread_info = Alpp2p_cms_main_ipcamera_thread_info->lpp2p_cms_ipcamera_channel_main_thread_info[thread_info_index];
    ...
    }
    {
        uint32_t thread_info_index;
        for (thread_info_index = 0; thread_info_index < P2P_CMS_IPCAMERA_CHANNEL_MAIN_THREAD_MAX; thread_info_index++) {
            if (p2p_cms_ipcamera_channel_main_free(Alpp2p_cms_main_ipcamera_thread_info, thread_info_index) != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
            }
        }
    }
}

static void p2p_cms_ipcamera_channel_main_init_callback(p2p_cms_ipcamera_channel_main_thread_info_t *Alpthread_info, libpiziot_p2p_common_channel_data_in_p Alpdata_in_callback) {
    memset_addr(libpiziot_p2p_clientsdk_channel_client_callback_t, &(m_p2p_cms_ipcamera_callback), 0, sizeof(m_p2p_cms_ipcamera_callback));
    {
        m_p2p_cms_ipcamera_callback.lpobject = Alpthread_info;
        m_p2p_cms_ipcamera_callback.break_callback.lpthread_instance = &(Alpthread_info->thread_instance);
        m_p2p_cms_ipcamera_callback.lpdata_in_callback = Alpdata_in_callback;
    }
}

libpiziot_os_type_func_result_e p2p_cms_ipcamera_channel_main_start_thread(p2p_cms_main_ipcamera_thread_info_t *Alpp2p_cms_main_ipcamera_thread_info, int32_t Aviewer_handle, int32_t Athread_info_index, int32_t Achannel_id, libpiziot_os_pthread_routine_p Alppthread_routine, libpiziot_p2p_common_channel_data_in_p Alpdata_in_callback, int32_t Asend_data_max_size, int32_t Arecv_data_max_size, p2p_func_client_channel_aes_e Aenable_channel_aes, libpiziot_p2p_common_relay_data_type_e Adata_type) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    if (Athread_info_index >= P2P_CMS_IPCAMERA_CHANNEL_MAIN_THREAD_MAX) {
        return rval;
    }
    do {
        if (p2p_cms_ipcamera_channel_main_malloc(Alpp2p_cms_main_ipcamera_thread_info, Athread_info_index, Asend_data_max_size, Arecv_data_max_size) != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
            break;
        }
        {
            p2p_cms_ipcamera_channel_main_thread_info_t *lpthread_info = Alpp2p_cms_main_ipcamera_thread_info->lpp2p_cms_ipcamera_channel_main_thread_info[Athread_info_index];
            ...
            }
            {
                ...
                p2p_cms_ipcamera_channel_main_init_callback(lpthread_info, Alpdata_in_callback);
                ...
                if (Alppthread_routine != 0) {
                    p2p_cms_ipcamera_channel_main_start_thread_one(Alpp2p_cms_main_ipcamera_thread_info, Athread_info_index, Alppthread_routine);
                }
            }
        }
        rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS;
...
libpiziot_os_type_func_result_e p2p_cms_ipcamera_channel_main_stop_thread(p2p_cms_main_ipcamera_thread_info_t *Alpp2p_cms_main_ipcamera_thread_info, int32_t Athread_info_index) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    do {
        p2p_cms_ipcamera_channel_main_stop_thread_one(Alpp2p_cms_main_ipcamera_thread_info, Athread_info_index);
        {
            p2p_cms_ipcamera_channel_main_thread_info_t *lpthread_info = Alpp2p_cms_main_ipcamera_thread_info->lpp2p_cms_ipcamera_channel_main_thread_info[Athread_info_index];
            if (lpthread_info == 0) continue;
            if (lpthread_info->p2p_cms_channel_client_handle >= 0) {
        ...
        if (p2p_cms_ipcamera_channel_main_free(Alpp2p_cms_main_ipcamera_thread_info, Athread_info_index) != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
        }
        rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS;
    } while (0);
    return rval;
...
void p2p_cms_ipcamera_channel_main_finalize(p2p_cms_main_ipcamera_thread_info_t *Alpp2p_cms_main_ipcamera_thread_info) {
    if (Alpp2p_cms_main_ipcamera_thread_info) {
        LIBPIZIOT_FIX_ANDROID_COMPILE_MIPS_ERROR(0);
    }
}

// Initialize channel client thread.
libpiziot_os_type_func_result_e p2p_cms_ipcamera_channel_main_initialize(p2p_cms_main_ipcamera_thread_info_t *Alpp2p_cms_main_ipcamera_thread_info) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS;
    do {
        {
            // Initialize channel client thread.
            uint32_t thread_info_index;
            for (thread_info_index = 0; thread_info_index < P2P_CMS_IPCAMERA_CHANNEL_MAIN_THREAD_MAX; thread_info_index++) {
                Alpp2p_cms_main_ipcamera_thread_info->lpp2p_cms_ipcamera_channel_main_thread_info[thread_info_index] = 0;
            }
        }
    } while (0);
    if (rval != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
        p2p_cms_ipcamera_channel_main_finalize(Alpp2p_cms_main_ipcamera_thread_info);
    }
    return rval;
}
