...
//Define this Header
#include "p2p_cms_nas_channel_command.h"
...
#if defined(P2P_FUNC_NAS_ENABLE_DATA_COMMAND_VIEWER_SEND_DATA)
static struct timeval p2p_cms_nas_channel_command_send_packet_timeout = { 30, 0 };
#endif //defined(P2P_FUNC_NAS_ENABLE_DATA_COMMAND_VIEWER_SEND_DATA)

void p2p_cms_nas_channel_command_data_in_callback(void *Alpobject, int32_t Aviewer_handle, int32_t Achannel_client_handle, int32_t Achannel_id, struct sockaddr_in *Alpin_channel_server_ip, unsigned char *Alpdata, int32_t Adata_size) {
    p2p_cms_nas_channel_main_thread_info_t *lpthread_info = Alpobject;
    if (lpthread_info == 0) return;
    if (Adata_size <= 0) return;
    do {
        if ((Adata_size > lpthread_info->data_dec_from_server_size) || (Adata_size & 0xF)) {
            TRACEA("PIZCmsNas:data from channel server command size error !\n");
            break;
        }
        {
            ...
            }
            {
                ...
                uint32_t identy = Achannel_id + NAS_DATA_SERVER_SEND_C;
                if (mem_endian_32(lpprotocol_head->identy) != identy) {
                    TRACEA("PIZCmsNas:data from channel server command crc error !\n");
                    break;
                }
...
static libpiziot_os_type_func_result_e p2p_cms_nas_channel_command_encrypt_data_callback_before_send(unsigned char *Alpdata, int32_t Adata_size, unsigned char *Alpdata_enc, unsigned char **Alppdata_send, void *Alpobject) {
...
static libpiziot_os_type_func_result_e p2p_cms_nas_channel_command_send(p2p_cms_nas_channel_main_thread_info_t *Alpthread_info, unsigned char *Alpdata_send, int32_t Adata_size, unsigned char *Alpdata_enc_to_server, libpiziot_p2p_common_encrypt_data_callback_before_send_p Alpencrypt_data_callback) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    ...
    do {
        ...
        {
            rval = p2p_cms_nas_channel_main_get_net_status(Alpthread_info->p2p_cms_channel_client_handle, Alpthread_info->channel_id, &(Alpthread_info->net_status));
            ...
            do {
...
#if defined(LIBPIZIOT_P2P_COMMON_ENABLE_WAN_MODE) || defined(LIBPIZIOT_P2P_COMMON_ENABLE_LAN_MODE)
                {
...
#endif //defined(SAMPLE_CODE_P2P_CHANNEL_DATA_CB)
                    {
                        ...
                        {
                            uint32_t identy = Alpthread_info->channel_id + NAS_DATA_CLIENT_SEND_C;
                            lpprotocol_head->identy = mem_endian_32(identy);
                        }
                        lpprotocol_head->sequence = mem_endian_32(Alpthread_info->send_counter);
                        rval = libpiziot_p2p_clientsdk_channel_client_send_data(Alpthread_info->p2p_cms_channel_client_handle, Alpdata_send, Adata_size, lpdata_enc, send_packet_bytes_per_second, p2p_cms_nas_channel_command_send_packet_timeout, Alpencrypt_data_callback, Alpthread_info);
                    }
                }
...
libpiziot_os_type_func_result_e p2p_cms_nas_channel_command_send_to_channel_server(p2p_cms_nas_channel_main_thread_info_t *Alpthread_info, unsigned char *Alpdata_send, int32_t Adata_size) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    if (Alpdata_send == 0 || Adata_size <= 0) return rval;
    if (Adata_size > 0) {
        int32_t enc_size = (Adata_size + 0xF) & 0xFFFFFFF0;
        if (enc_size > p2p_protocol_nas_command_viewer_send_data_max_size) {
            TRACEA("out of command size(%d) - %d = %d\n", enc_size, p2p_protocol_nas_command_viewer_send_data_max_size, enc_size - p2p_protocol_nas_command_viewer_send_data_max_size);
            return rval;
        }
...
libpiziot_os_pthread_dword_t p2p_cms_nas_channel_command_thread_instance_routine(void *arg) {
    libpiziot_os_pthread_instance_t *lpthread_instance = (libpiziot_os_pthread_instance_t *)arg;
    p2p_cms_nas_channel_main_thread_info_t *lpthread_info = (p2p_cms_nas_channel_main_thread_info_t *)(lpthread_instance->m_extra);
    ...
    do {
            ...
            if (align_data_len > 0) {
                ...
                for (cmd_index = 0; cmd_index < cmd_count; cmd_index++) {
                    ...
                    if (offset >= align_data_len) break;
                    {
                        ...
                        if (p2p_cms_nas_channel_command_common_parser(cmd, lpthread_info->thread_index, lpcmd) == LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) continue;
                        ...
                        if (p2p_cms_nas_channel_command_security_system_parser(cmd, lpthread_info->thread_index, lpcmd) == LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) continue;
...

        {
            ...
            do {
                ...
                {
                    libpiziot_os_type_func_result_e ret = p2p_cms_nas_channel_main_get_net_status(lpthread_info->p2p_cms_channel_client_handle, lpthread_info->channel_id, &(lpthread_info->net_status));
                ...
                case LIBPIZIOT_P2P_COMMON_RESULT_MODE_WAN_CHANNEL_IN_RELAY: {
                    ...
                    p2p_cms_nas_channel_command_common_send_action(lpthread_info->thread_index);

#endif //defined(LIBPIZIOT_CORE_P2P_PROTOCOL_NAS_COMMAND_COMMON)

#if defined(LIBPIZIOT_CORE_P2P_PROTOCOL_NAS_COMMAND_SECURITY_SYSTEM)

                    p2p_cms_nas_channel_command_security_system_send_action(lpthread_info->thread_index);
        ...
        if (lpthread_info->lpdata_send_to_server != 0 && lpthread_info->lpdata_enc_to_server != 0) {
            ...
            while (cmd_len > 0) {
                ...
                if (cmd_len > 0) {
                    ...
                    cmd_count++;
                    {
                        ...
                        if ((offset + cmd_encrypt_len) >= p2p_protocol_nas_command_viewer_send_data_max_size) {
                            break;
                        }
                    }
                }
            ...
            if (cmd_count > 0) {
                ...
                p2p_cms_nas_channel_command_send(lpthread_info, (unsigned char *)lpprotocol_head, data_len, lpthread_info->lpdata_enc_to_server, p2p_cms_nas_channel_command_encrypt_data_callback_before_send);
            }
        }
#endif //defined(P2P_FUNC_NAS_ENABLE_DATA_COMMAND_VIEWER_SEND_DATA)
    } while (1);
