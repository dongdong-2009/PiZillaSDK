...
#include "../p2p_cms_channel_client_main/nas_data/p2p_cms_nas_channel_command.h"
#include "../p2p_cms_channel_client_main/nas_data/p2p_cms_nas_channel_other.h"

#if defined(LIBPIZIOT_CORE_P2P_PROTOCOL_NAS_COMMAND)
#if defined(LIBPIZIOT_CORE_P2P_PROTOCOL_NAS_COMMAND_COMMON)
#include "../p2p_cms_channel_client_main/nas_data/command/p2p_cms_nas_channel_command_common.h"
...
#include "../p2p_cms_channel_client_main/nas_data/command/p2p_cms_nas_channel_command_security_system.h"
...
static libpiziot_os_type_func_result_e p2p_cms_main_nas_get_net_status(int32_t Aviewer_handle, libpiziot_p2p_common_result_e *Alpviewer_net_status) {
    ...
    do {
        ...
        if (viewer_net_result != (*Alpviewer_net_status)) {
            ...
            case LIBPIZIOT_P2P_COMMON_RESULT_CONNECTING: {
                TRACEA("PIZCmsNas:VIEWER HANDLE:(%d) CONNECTING\n", Aviewer_handle);
            }
                                                        break;
            case LIBPIZIOT_P2P_COMMON_RESULT_MODE_LAN_DEVICE_IN_LAN: {
                TRACEA("PIZCmsNas:VIEWER HANDLE:(%d) LAN_DEVICE_IN_LAN\n", Aviewer_handle);
            }
                                                                    break;
            case LIBPIZIOT_P2P_COMMON_RESULT_MODE_LAN_DEVICE_IN_WAN: {
                TRACEA("PIZCmsNas:VIEWER HANDLE:(%d) LAN_DEVICE_IN_WAN\n", Aviewer_handle);
            }
                                                                    break;
            case LIBPIZIOT_P2P_COMMON_RESULT_MODE_WAN_DEVICE: {
                TRACEA("PIZCmsNas:VIEWER HANDLE:(%d) WAN_DEVICE\n", Aviewer_handle);
            }
                                                             break;
            default: {
                TRACEA("PIZCmsNas:VIEWER HANDLE:(%d) ERROR !! = %d\n", Aviewer_handle, viewer_net_result);
            }
                     break;
            }
        }
...
static void p2p_cms_main_nas_channel_update_mode(p2p_cms_main_nas_thread_info_t *Alpthread_info, int32_t Aviewer_handle, int32_t Achannel_id, libpiziot_os_pthread_routine_p Alppthread_routine, libpiziot_p2p_common_channel_data_in_p Alpdata_in_callback, int32_t Asend_data_max_size, int32_t Arecv_data_max_size, p2p_func_client_channel_aes_e Aenable_channel_aes, libpiziot_p2p_common_relay_data_type_e Adata_type, p2p_cms_nas_channel_main_protocol_command_t *Alpprotocol_command) {
    ...
    if (mode == P2P_FUNC_CLIENT_CHANNEL_ENABLE_YES) {
        p2p_cms_nas_channel_main_start_thread(Alpthread_info, Aviewer_handle, Alpthread_info->update_thread_info_index, Achannel_id, Alppthread_routine, Alpdata_in_callback, Asend_data_max_size, Arecv_data_max_size, Aenable_channel_aes, Adata_type);
        {
            libpiziot_os_mutex_plock_lock(&p2p_cms_main_nas_channel_mutex);
            {
                Alpprotocol_command->lpthread_info = Alpthread_info->lpp2p_cms_nas_channel_main_thread_info[Alpthread_info->update_thread_info_index];
#if defined(LIBPIZIOT_CORE_P2P_PROTOCOL_NAS_COMMAND)
#if defined(LIBPIZIOT_CORE_P2P_PROTOCOL_NAS_COMMAND_COMMON)
                {
                    p2p_cms_nas_channel_command_common_init_action(Alpprotocol_command);
                }
#endif //defined(LIBPIZIOT_CORE_P2P_PROTOCOL_NAS_COMMAND_COMMON)
#if defined(LIBPIZIOT_CORE_P2P_PROTOCOL_NAS_COMMAND_SECURITY_SYSTEM)
                {
                    p2p_cms_nas_channel_command_security_system_init_action(Alpprotocol_command);
                }
#endif //defined(LIBPIZIOT_CORE_P2P_PROTOCOL_NAS_COMMAND_SECURITY_SYSTEM)
#endif //defined(LIBPIZIOT_CORE_P2P_PROTOCOL_NAS_COMMAND)
            }
    ...
    if (mode == P2P_FUNC_CLIENT_CHANNEL_ENABLE_NO) {
        ...
        libpiziot_os_mutex_plock_unlock(&p2p_cms_main_nas_channel_mutex);
        p2p_cms_nas_channel_main_stop_thread(Alpthread_info, Alpthread_info->update_thread_info_index);
    }
    Alpthread_info->update_thread_info_index++;
}

static void p2p_cms_main_nas_start_channel_thread(p2p_cms_main_nas_thread_info_t *Alpthread_info) {
    if (Alpthread_info) {
        Alpthread_info->update_thread_info_index = 0;
#if defined(LIBPIZIOT_CORE_P2P_PROTOCOL_NAS_COMMAND)
        p2p_cms_main_nas_channel_update_mode(Alpthread_info, Alpthread_info->p2p_cms_main_nas_handle, LIBPIZIOT_CORE_P2P_PROTOCOL_NAS_COMMAND, p2p_cms_nas_channel_command_thread_instance_routine, p2p_cms_nas_channel_command_data_in_callback, p2p_protocol_nas_command_viewer_send_data_max_size, p2p_protocol_nas_command_viewer_recv_data_max_size, P2P_FUNC_CLIENT_CHANNEL_AES_ENABLE, LIBPIZIOT_P2P_COMMON_RELAY_DATA_TYPE_DATA, &(Alpthread_info->protocol_command));
#endif //defined(LIBPIZIOT_CORE_P2P_PROTOCOL_NAS_COMMAND)
...
//20, LIBPIZIOT_CORE_P2P_NAS_CHANNEL_04_PLAYBACK_AUDIO_TO_APP
        ...
        p2p_cms_main_nas_channel_update_mode(Alpthread_info, Alpthread_info->p2p_cms_main_nas_handle, LIBPIZIOT_CORE_P2P_PROTOCOL_NAS_OTHER, p2p_cms_nas_channel_other_thread_instance_routine, p2p_cms_nas_channel_other_data_in_callback, p2p_protocol_nas_other_viewer_send_data_max_size, p2p_protocol_nas_other_viewer_recv_data_max_size, P2P_FUNC_CLIENT_CHANNEL_AES_ENABLE, LIBPIZIOT_P2P_COMMON_RELAY_DATA_TYPE_DATA, &(Alpthread_info->protocol_other));
#endif //defined(LIBPIZIOT_CORE_P2P_PROTOCOL_NAS_OTHER)
    }
}

static void p2p_cms_main_nas_stop_channel_thread(p2p_cms_main_nas_thread_info_t *Alpthread_info) {
    p2p_cms_nas_channel_main_stop_thread_all(Alpthread_info);
}

static void p2p_cms_main_nas_create_p2p_viewer(p2p_cms_main_nas_thread_info_t *Alpthread_info, const char *AlpUID) {
...
#define CONNECT_SERVER_SOCKET_INFO_COUNT 1
        ...
        if (p2p_func_client_viewer_get_nas_device_key(&(Alpthread_info->login_aes.aes_key), Alpthread_info->login_aes.cbc_iv, device_account, LIBPIZIOT_P2P_COMMON_DEVICE_ACCOUNT_LENGTH_MAX, device_password, LIBPIZIOT_P2P_COMMON_DEVICE_PASSWORD_LENGTH_MAX) != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) return;

        if (libpiziot_p2p_clientsdk_create_viewer(connect_server_socket_info, CONNECT_SERVER_SOCKET_INFO_COUNT, SAMPLE_CODE_P2P_LAN_SERVER_PORT_DEFAULT, AlpUID, select_net_type, device_account, device_password, &(Alpthread_info->p2p_cms_main_nas_handle)) == LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
        }
    }
}

static void p2p_cms_main_nas_delete_p2p_viewer(p2p_cms_main_nas_thread_info_t *Alpthread_info) {
    p2p_cms_main_nas_stop_channel_thread(Alpthread_info);
    if (Alpthread_info->p2p_cms_main_nas_handle >= 0) {
        libpiziot_p2p_clientsdk_delete_viewer(Alpthread_info->p2p_cms_main_nas_handle);
        Alpthread_info->p2p_cms_main_nas_handle = -1;
    }
}

static libpiziot_os_pthread_dword_t p2p_cms_main_nas_thread_instance_routine(void *arg) {
    libpiziot_os_pthread_instance_t *lpthread_instance = (libpiziot_os_pthread_instance_t *)arg;
    p2p_cms_main_nas_thread_info_t *lpthread_info = (p2p_cms_main_nas_thread_info_t *)(lpthread_instance->m_extra);
...

    {
        ...
        do {
            ...
            if (lpthread_info->p2p_cms_main_nas_handle == -1) {
                usleep(1000000);
                p2p_cms_main_nas_create_p2p_viewer(lpthread_info, lpthread_info->lpuuid);
                continue;
            }
            else {
                p2p_cms_main_nas_start_channel_thread(lpthread_info);
            }
            do {
                libpiziot_os_type_func_result_e rval = p2p_cms_main_nas_get_net_status(lpthread_info->p2p_cms_main_nas_handle, &viewer_net_result);
                if (rval != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
                    usleep(500000);
                    break;
                }
    ...
    p2p_cms_main_nas_delete_p2p_viewer(lpthread_info);

    return (libpiziot_os_pthread_dword_t)1234567;
}

libpiziot_os_type_func_result_e p2p_cms_main_nas_connect_device(p2p_cms_main_nas_thread_info_t *Alpthread_info, const char *AlpUID) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    if (Alpthread_info == 0) return rval;
    {
        ...
        do {
            ...
            if ((libpiziot_os_pthread_get_status(&(Alpthread_info->thread_instance)) == LIBPIZIOT_OS_PTHREAD_STATUS_CLOSED)) {
                ...
                if (libpiziot_os_pthread_create(&(Alpthread_info->thread_instance), p2p_cms_main_nas_thread_instance_routine, LIBPIZIOT_OS_PTHREAD_STACK_SIZE_AUTO, "core_cms_nas") != 0) {
...
void p2p_cms_main_nas_disconnect_device(p2p_cms_main_nas_thread_info_t *Alpthread_info) {
...
libpiziot_os_type_func_result_e p2p_cms_main_nas_finalize(p2p_cms_main_nas_thread_info_t **Alpp_thread_info) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS;
    do {
        p2p_cms_main_nas_thread_info_t *lpthread_info = (*Alpp_thread_info);
        if (lpthread_info == 0) break;
        if (lpthread_info->instance_mutex.init != LIBPIZIOT_OS_MUTEX_STUCT_INIT_CHECK_VALUE) break;
        {
            p2p_cms_nas_channel_main_finalize(lpthread_info);
        }
        {
...
libpiziot_os_type_func_result_e p2p_cms_main_nas_initialize(p2p_cms_main_nas_thread_info_t **Alpp_thread_info) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    p2p_cms_main_nas_thread_info_t *lpthread_info = 0;
    do {
...
#else
        {
            ...
            if ((free_memory_size + sizeof(p2p_cms_main_nas_thread_info_t)) < LIBPIZIOT_P2P_COMMON_SYSTEM_MEMORY_MINIMUM_SIZE) {
        ...
        lpthread_info = (p2p_cms_main_nas_thread_info_t *)libpiziot_os_malloc(sizeof(p2p_cms_main_nas_thread_info_t));
        if (lpthread_info == 0) {
        ...
        {
            memset_addr(p2p_cms_main_nas_thread_info_t, lpthread_info, 0, sizeof(p2p_cms_main_nas_thread_info_t));
            lpthread_info->p2p_cms_main_nas_handle = -1;
#if defined(LIBPIZIOT_CORE_P2P_PROTOCOL_NAS_COMMAND)
            ...
            lpthread_info->thread_instance.m_extra = lpthread_info;
            {
                ...
                {
                    if (p2p_cms_nas_channel_main_initialize(lpthread_info) != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
                        break;
                    }
                }
            }
    ...
    if (rval != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
        p2p_cms_main_nas_finalize(Alpp_thread_info);
        TRACEA("%s,error !!\n", LIBPIZIOT_OS__FUNCTION__);
    }
    return rval;
}
