...
static libpiziot_os_type_func_result_e p2p_cms_main_ipcamera_get_net_status(int32_t Aviewer_handle, libpiziot_p2p_common_result_e *Alpviewer_net_status) {
    ...
    do {
        ...
        if (viewer_net_result != (*Alpviewer_net_status)) {
            ...
            case LIBPIZIOT_P2P_COMMON_RESULT_CONNECTING: {
                TRACEA("PIZCmsCam:VIEWER HANDLE:(%d) CONNECTING\n", Aviewer_handle);
            }
                                                        break;
            case LIBPIZIOT_P2P_COMMON_RESULT_MODE_LAN_DEVICE_IN_LAN: {
                TRACEA("PIZCmsCam:VIEWER HANDLE:(%d) LAN_DEVICE_IN_LAN\n", Aviewer_handle);
            }
                                                                    break;
            case LIBPIZIOT_P2P_COMMON_RESULT_MODE_LAN_DEVICE_IN_WAN: {
                TRACEA("PIZCmsCam:VIEWER HANDLE:(%d) LAN_DEVICE_IN_WAN\n", Aviewer_handle);
            }
                                                                    break;
            case LIBPIZIOT_P2P_COMMON_RESULT_MODE_WAN_DEVICE: {
                TRACEA("PIZCmsCam:VIEWER HANDLE:(%d) WAN_DEVICE\n", Aviewer_handle);
            }
                                                             break;
            default: {
                TRACEA("PIZCmsCam:VIEWER HANDLE:(%d) ERROR !! = %d\n", Aviewer_handle, viewer_net_result);
            }
                     break;
            }
        }
...
static void p2p_cms_main_ipcamera_start_channel_thread(p2p_cms_main_ipcamera_thread_info_t *Alpthread_info) {
    if (Alpthread_info) {
...
#define CONNECT_SERVER_SOCKET_INFO_COUNT 1
        ...
        if (libpiziot_p2p_clientsdk_create_viewer(connect_server_socket_info, CONNECT_SERVER_SOCKET_INFO_COUNT, SAMPLE_CODE_P2P_LAN_SERVER_PORT_DEFAULT, AlpUID, select_net_type, device_account, device_password, &(Alpthread_info->p2p_cms_main_ipcamera_handle)) == LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
        }
    }
}

static void p2p_cms_main_ipcamera_delete_p2p_viewer(p2p_cms_main_ipcamera_thread_info_t *Alpthread_info) {
    p2p_cms_main_ipcamera_stop_channel_thread(Alpthread_info);
    if (Alpthread_info->p2p_cms_main_ipcamera_handle >= 0) {
        libpiziot_p2p_clientsdk_delete_viewer(Alpthread_info->p2p_cms_main_ipcamera_handle);
        Alpthread_info->p2p_cms_main_ipcamera_handle = -1;
    }
}

static libpiziot_os_pthread_dword_t p2p_cms_main_ipcamera_thread_instance_routine(void *arg) {
    libpiziot_os_pthread_instance_t *lpthread_instance = (libpiziot_os_pthread_instance_t *)arg;
    p2p_cms_main_ipcamera_thread_info_t *lpthread_info = (p2p_cms_main_ipcamera_thread_info_t *)(lpthread_instance->m_extra);
...

    {
        ...
        do {
            ...
            if (lpthread_info->p2p_cms_main_ipcamera_handle == -1) {
                usleep(1000000);
                p2p_cms_main_ipcamera_create_p2p_viewer(lpthread_info, lpthread_info->lpuuid);
                continue;
            }
            else {
                p2p_cms_main_ipcamera_start_channel_thread(lpthread_info);
            }
            do {
                libpiziot_os_type_func_result_e rval = p2p_cms_main_ipcamera_get_net_status(lpthread_info->p2p_cms_main_ipcamera_handle, &viewer_net_result);
                if (rval != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
                    usleep(500000);
                    break;
                }
    ...
    p2p_cms_main_ipcamera_delete_p2p_viewer(lpthread_info);

    return (libpiziot_os_pthread_dword_t)1234567;
}

libpiziot_os_type_func_result_e p2p_cms_main_ipcamera_connect_device(p2p_cms_main_ipcamera_thread_info_t *Alpthread_info, const char *AlpUID) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    if (Alpthread_info == 0) return rval;
    {
        ...
        do {
            ...
            if ((libpiziot_os_pthread_get_status(&(Alpthread_info->thread_instance)) == LIBPIZIOT_OS_PTHREAD_STATUS_CLOSED)) {
                ...
                if (libpiziot_os_pthread_create(&(Alpthread_info->thread_instance), p2p_cms_main_ipcamera_thread_instance_routine, LIBPIZIOT_OS_PTHREAD_STACK_SIZE_AUTO, "core_cms_cam") != 0) {
...
void p2p_cms_main_ipcamera_disconnect_device(p2p_cms_main_ipcamera_thread_info_t *Alpthread_info) {
...
libpiziot_os_type_func_result_e p2p_cms_main_ipcamera_finalize(p2p_cms_main_ipcamera_thread_info_t **Alpp_thread_info) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS;
    do {
        p2p_cms_main_ipcamera_thread_info_t *lpthread_info = (*Alpp_thread_info);
...
libpiziot_os_type_func_result_e p2p_cms_main_ipcamera_initialize(p2p_cms_main_ipcamera_thread_info_t **Alpp_thread_info) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    p2p_cms_main_ipcamera_thread_info_t *lpthread_info = 0;
    do {
...
#else
        {
            ...
            if ((free_memory_size + sizeof(p2p_cms_main_ipcamera_thread_info_t)) < LIBPIZIOT_P2P_COMMON_SYSTEM_MEMORY_MINIMUM_SIZE) {
        ...
        lpthread_info = (p2p_cms_main_ipcamera_thread_info_t *)libpiziot_os_malloc(sizeof(p2p_cms_main_ipcamera_thread_info_t));
        if (lpthread_info == 0) {
        ...
        {
            memset_addr(p2p_cms_main_ipcamera_thread_info_t, lpthread_info, 0, sizeof(p2p_cms_main_ipcamera_thread_info_t));
            lpthread_info->p2p_cms_main_ipcamera_handle = -1;
#if defined(LIBPIZIOT_CORE_P2P_PROTOCOL_IPCAMERA_COMMAND)
    ...
    if (rval != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
        p2p_cms_main_ipcamera_finalize(Alpp_thread_info);
        TRACEA("%s,error !!\n", LIBPIZIOT_OS__FUNCTION__);
    }
    return rval;
}
