...
//Define this Header
#include "p2p_nas_ipcamera_channel_other.h"
...
static struct timeval p2p_nas_ipcamera_channel_other_send_packet_timeout = { 30, 0 };

void p2p_nas_ipcamera_channel_other_data_in_callback(void *Alpobject, int32_t Aviewer_handle, int32_t Achannel_client_handle, int32_t Achannel_id, struct sockaddr_in *Alpin_channel_server_ip, unsigned char *Alpdata, int32_t Adata_size) {
    p2p_nas_ipcamera_channel_main_thread_info_t *lpthread_info = Alpobject;
    if (lpthread_info == 0) return;
    do {
        if (Adata_size != p2p_protocol_ipcamera_other_device_send_data_max_size) {
            TRACEA("PIZNasCam:data size error !\n");
            break;
        }
        {
            ...
            }
            {
                ...
                for (pos = 0; pos < data_size; pos++) {
                    if (lpChar[pos] != c) {
                        TRACEA("PIZNasCam:data crc error !\n");
                        data_size = 0;
                        break;
                    }
                    c++;
...
static libpiziot_os_type_func_result_e p2p_nas_ipcamera_channel_other_encrypt_data_callback_before_send(unsigned char *Alpdata, int32_t Adata_size, unsigned char *Alpdata_enc, unsigned char **Alppdata_send, void *Alpobject) {
...
static libpiziot_os_type_func_result_e p2p_nas_ipcamera_channel_other_send(p2p_nas_ipcamera_channel_main_thread_info_t *Alpthread_info, unsigned char *Alpdata_send, int32_t Adata_size, unsigned char *Alpdata_enc_to_server, libpiziot_p2p_common_encrypt_data_callback_before_send_p Alpencrypt_data_callback) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    ...
    do {
        ...
        {
            rval = p2p_nas_ipcamera_channel_main_get_net_status(Alpthread_info->p2p_nas_channel_client_handle, Alpthread_info->channel_id, &(Alpthread_info->net_status));
            ...
            do {
...
#if defined(LIBPIZIOT_P2P_COMMON_ENABLE_WAN_MODE) || defined(LIBPIZIOT_P2P_COMMON_ENABLE_LAN_MODE)
                {
...
#endif //defined(SAMPLE_CODE_P2P_CHANNEL_DATA_CB)
                    {
                        ...
                        rval = libpiziot_p2p_clientsdk_channel_client_send_data(Alpthread_info->p2p_nas_channel_client_handle, Alpdata_send, Adata_size, lpdata_enc, send_packet_bytes_per_second, p2p_nas_ipcamera_channel_other_send_packet_timeout, Alpencrypt_data_callback, Alpthread_info);
                    }
                }
...
static void p2p_nas_ipcamera_channel_other_init_data(int32_t Achannel_id, unsigned char *Alpdata_send, int32_t Adata_size) {
    int32_t pos;
...
libpiziot_os_type_func_result_e p2p_nas_ipcamera_channel_other_send_to_channel_server(p2p_nas_ipcamera_channel_main_thread_info_t *Alpthread_info, unsigned char *Alpdata_send, int32_t Adata_size) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    if (Adata_size > p2p_protocol_ipcamera_other_viewer_send_data_max_size) return rval;
    rval = p2p_nas_ipcamera_channel_other_send(Alpthread_info, Alpdata_send, Adata_size, Alpthread_info->lpdata_enc_to_server, p2p_nas_ipcamera_channel_other_encrypt_data_callback_before_send);
    return rval;
}

libpiziot_os_pthread_dword_t p2p_nas_ipcamera_channel_other_thread_instance_routine(void *arg) {
    libpiziot_os_pthread_instance_t *lpthread_instance = (libpiziot_os_pthread_instance_t *)arg;
    p2p_nas_ipcamera_channel_main_thread_info_t *lpthread_info = (p2p_nas_ipcamera_channel_main_thread_info_t *)(lpthread_instance->m_extra);

    if (lpthread_info->lpdata_send_to_server != 0 && lpthread_info->lpdata_enc_to_server != 0) {
        p2p_nas_ipcamera_channel_other_init_data(lpthread_info->channel_id, lpthread_info->lpdata_send_to_server, p2p_protocol_ipcamera_other_viewer_send_data_max_size);
    }
    ...
    do {
        ...
        if (lpthread_info->lpdata_send_to_server != 0 && lpthread_info->lpdata_enc_to_server != 0) {
            p2p_nas_ipcamera_channel_other_send(lpthread_info, lpthread_info->lpdata_send_to_server, p2p_protocol_ipcamera_other_viewer_send_data_max_size, lpthread_info->lpdata_enc_to_server, p2p_nas_ipcamera_channel_other_encrypt_data_callback_before_send);
        }
#if 0
        if (lpthread_info->net_status < 0)
        {
            TRACEA("PIZNasCam:DISCONNECT CHANNEL HANDLE:(%d),ID:(%d)\n", lpthread_info->p2p_nas_channel_client_handle, lpthread_info->channel_id);
