...
//Define this Header
#include "p2p_nas_ipcamera_channel_command.h"
...
static struct timeval p2p_nas_ipcamera_channel_command_send_packet_timeout = { 30, 0 };
#endif //defined(P2P_PROTOCOL_IPCAMERA_COMMAND_VIEWER_SEND_DATA_MAX_SIZE)

void p2p_nas_ipcamera_channel_command_data_in_callback(void *Alpobject, int32_t Aviewer_handle, int32_t Achannel_client_handle, int32_t Achannel_id, struct sockaddr_in *Alpin_channel_server_ip, unsigned char *Alpdata, int32_t Adata_size) {
...
static libpiziot_os_type_func_result_e p2p_nas_ipcamera_channel_command_encrypt_data_callback_before_send(unsigned char *Alpdata, int32_t Adata_size, unsigned char *Alpdata_enc, unsigned char **Alppdata_send, void *Alpobject) {
...
static libpiziot_os_type_func_result_e p2p_nas_ipcamera_channel_command_send(p2p_nas_ipcamera_channel_main_thread_info_t *Alpthread_info, unsigned char *Alpdata_send, int32_t Adata_size, unsigned char *Alpdata_enc_to_server, libpiziot_p2p_common_encrypt_data_callback_before_send_p Alpencrypt_data_callback) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
            ...
            do {
...
#if defined(LIBPIZIOT_P2P_COMMON_ENABLE_WAN_MODE) || defined(LIBPIZIOT_P2P_COMMON_ENABLE_LAN_MODE)
                {
...
#endif //defined(SAMPLE_CODE_P2P_CHANNEL_DATA_CB)
                    {
                        ...
                        rval = libpiziot_p2p_clientsdk_channel_client_send_data(Alpthread_info->p2p_nas_channel_client_handle, Alpdata_send, Adata_size, lpdata_enc, send_packet_bytes_per_second, p2p_nas_ipcamera_channel_command_send_packet_timeout, Alpencrypt_data_callback, Alpthread_info);
                    }
                }
...
libpiziot_os_type_func_result_e p2p_nas_ipcamera_channel_command_send_to_channel_server(p2p_nas_ipcamera_channel_main_thread_info_t *Alpthread_info, unsigned char *Alpdata_send, int32_t Adata_size) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
...
libpiziot_os_pthread_dword_t p2p_nas_ipcamera_channel_command_thread_instance_routine(void *arg) {
    ...
    do {
...
#if defined(LIBPIZIOT_CORE_P2P_PROTOCOL_IPCAMERA_COMMAND_COMMON)
                        if (p2p_nas_ipcamera_channel_command_common_parser(cmd, lpthread_info->thread_index, lpcmd) == LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) continue;
...

        {
            ...
            do {
                ...
                {
                    libpiziot_os_type_func_result_e ret = p2p_nas_ipcamera_channel_main_get_net_status(lpthread_info->p2p_nas_channel_client_handle, lpthread_info->channel_id, &(lpthread_info->net_status));
                ...
                case LIBPIZIOT_P2P_COMMON_RESULT_MODE_WAN_CHANNEL_IN_RELAY: {
                    ...
                    p2p_nas_ipcamera_channel_command_common_send_action(lpthread_info->thread_index);
        ...
        if (lpthread_info->lpdata_send_to_server != 0 && lpthread_info->lpdata_enc_to_server != 0) {
            ...
            if (cmd_count > 0) {
                ...
                p2p_nas_ipcamera_channel_command_send(lpthread_info, (unsigned char *)lpprotocol_head, data_len, lpthread_info->lpdata_enc_to_server, p2p_nas_ipcamera_channel_command_encrypt_data_callback_before_send);
            }
        }
#endif //defined(P2P_PROTOCOL_IPCAMERA_COMMAND_VIEWER_SEND_DATA_MAX_SIZE)
    } while (1);
