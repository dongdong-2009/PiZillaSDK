...
//Define this Header
#include "p2p_nas_ipcamera_other.h"
...
static libpiziot_os_type_func_result_e p2p_nas_ipcamera_other_send(p2p_nas_ipcamera_main_thread_info_t *Alpthread_info, unsigned char *Alpdata_send, int32_t Adata_size, unsigned char *Alpdata_enc_to_server, libpiziot_p2p_common_encrypt_data_callback_before_send_p Alpencrypt_data_callback) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
            ...
            do {
...
#if defined(LIBPIZIOT_P2P_COMMON_ENABLE_WAN_MODE) || defined(LIBPIZIOT_P2P_COMMON_ENABLE_LAN_MODE)
                {
...
#endif //defined(SAMPLE_CODE_P2P_CHANNEL_DATA_CB)
                    {
                        ...
                        rval = libpiziot_p2p_clientsdk_channel_client_send_data(Alpthread_info->p2p_nas_channel_client_handle, Alpdata_send, Adata_size, lpdata_enc, send_packet_bytes_per_second, p2p_nas_ipcamera_other_send_packet_timeout, Alpencrypt_data_callback, Alpthread_info);
                    }
                }
...
static void p2p_nas_ipcamera_other_init_data(int32_t Achannel_id, unsigned char *Alpdata_send, int32_t Adata_size) {
    int32_t pos;
...
libpiziot_os_type_func_result_e p2p_nas_ipcamera_other_send_to_channel_server(p2p_nas_ipcamera_main_thread_info_t *Alpthread_info, unsigned char *Alpdata_send, int32_t Adata_size) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    if (Adata_size > P2P_PROTOCOL_IPCAMERA_OTHER_VIEWER_SEND_DATA_MAX_SIZE) return rval;
    rval = p2p_nas_ipcamera_other_send(Alpthread_info, Alpdata_send, Adata_size, Alpthread_info->lpdata_enc_to_server, p2p_nas_ipcamera_other_encrypt_data_callback_before_send);
    return rval;
}
