...
//Define this Header
#include "p2p_nas_device_channel_other.h"
...
static struct timeval p2p_nas_device_channel_other_send_packet_timeout = { 30, 0 };

void p2p_nas_device_channel_other_data_in_callback(void *Alpobject, int32_t Adevice_handle, int32_t Achannel_server_handle, int32_t Achannel_id, struct sockaddr_in *Alpin_channel_client_ip, unsigned char *Alpdata, int32_t Adata_size) {
...
static libpiziot_os_type_func_result_e p2p_nas_device_channel_other_encrypt_data_callback_before_send(unsigned char *Alpdata, int32_t Adata_size, unsigned char *Alpdata_enc, unsigned char **Alppdata_send, void *Alpobject) {
...
static libpiziot_os_type_func_result_e p2p_nas_device_channel_other_send(p2p_nas_device_channel_main_thread_info_t *Alpthread_info, uint32_t Achannel_server_handle, unsigned char *Alpdata_send, int32_t Adata_size, libpiziot_p2p_common_encrypt_data_callback_before_send_p Alpencrypt_data_callback) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    ...
    do {
...
#if defined(LIBPIZIOT_P2P_COMMON_ENABLE_WAN_MODE) || defined(LIBPIZIOT_P2P_COMMON_ENABLE_LAN_MODE)
        {
            ...
            rval = libpiziot_p2p_clientsdk_channel_server_send_data(Alpthread_info->p2p_channel_monitor_handle, Achannel_server_handle, Alpdata_send, Adata_size, send_packet_bytes_per_second, p2p_nas_device_channel_other_send_packet_timeout, Alpencrypt_data_callback, Alpthread_info, &(Alpthread_info->net_status));
        }
...
static void p2p_nas_device_channel_other_init_data(int32_t Achannel_id, unsigned char *Alpdata_send, int32_t Adata_size) {
    int32_t pos;
...
libpiziot_os_type_func_result_e p2p_nas_device_channel_other_send_to_channel_client(p2p_nas_device_channel_main_thread_info_t *Alpthread_info, uint32_t Achannel_server_handle, unsigned char *Alpdata_send, int32_t Adata_size) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    if (Adata_size > p2p_protocol_nas_other_device_send_data_max_size) return rval;
    rval = p2p_nas_device_channel_other_send(Alpthread_info, Achannel_server_handle, Alpdata_send, Adata_size, p2p_nas_device_channel_other_encrypt_data_callback_before_send);
    return rval;
}

libpiziot_os_pthread_dword_t p2p_nas_device_channel_other_thread_instance_routine(void *arg) {
    ...
    if (lpthread_info->lpdata_send_to_client != 0) {
        p2p_nas_device_channel_other_init_data(lpthread_info->channel_id, lpthread_info->lpdata_send_to_client, p2p_protocol_nas_other_device_send_data_max_size);
    }
    ...
    do {
        ...
        if (lpthread_info->lpdata_send_to_client != 0) {
            p2p_nas_device_channel_other_send(lpthread_info, LIBPIZIOT_P2P_COMMON_CHANNEL_SERVER_BROADCAST_DATA, lpthread_info->lpdata_send_to_client, p2p_protocol_nas_other_device_send_data_max_size, p2p_nas_device_channel_other_encrypt_data_callback_before_send);
        }
