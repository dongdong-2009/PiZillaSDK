#ifndef __DEF_LIBPIZIOT_CORE_NAS_H__
#define __DEF_LIBPIZIOT_CORE_NAS_H__
...
typedef long (__stdcall *libpiziot_core_nas_event_p)(unsigned int AnIOCtrlType, unsigned char* AlpChar, uint32_t Asize);
#else
...
#if defined(__LIBPIZIOT_OS_PLATFORM_MFC__)
    extern libpiziot_core_nas_event_p lplibpiziot_core_nas_event;
#else
#endif //defined(__LIBPIZIOT_OS_PLATFORM_MFC__)

    extern libpiziot_os_type_func_result_e libpiziot_core_nas_initialize(void);
    extern void libpiziot_core_nas_finalize(void);
    extern void libpiziot_core_nas_sigterm_handler(int sig);
...
#else
    extern void libpiziot_core_nas_event_loop(void);
...
#endif //__DEF_LIBPIZIOT_CORE_NAS_H__
