#ifndef __DEF_LIBPIZIOT_CORE_UMBO_H__
#define __DEF_LIBPIZIOT_CORE_UMBO_H__
...
typedef long(__stdcall *libpiziot_core_umbo_event_p)(unsigned int AnIOCtrlType, unsigned char* AlpChar, uint32_t Asize);
#else
...
#if defined(__LIBPIZIOT_OS_PLATFORM_MFC__)
    extern libpiziot_core_umbo_event_p lplibpiziot_core_umbo_event;
#else
#endif //defined(__LIBPIZIOT_OS_PLATFORM_MFC__)

    extern libpiziot_os_type_func_result_e libpiziot_core_umbo_initialize(void);
    extern void libpiziot_core_umbo_finalize(void);
    extern void libpiziot_core_umbo_sigterm_handler(int sig);
...
#else
    extern void libpiziot_core_umbo_event_loop(void);
...
#endif //__DEF_LIBPIZIOT_CORE_UMBO_H__
