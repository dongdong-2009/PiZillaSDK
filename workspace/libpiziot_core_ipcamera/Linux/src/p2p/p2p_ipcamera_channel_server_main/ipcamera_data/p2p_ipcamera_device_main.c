...
//Define this Header
#include "p2p_ipcamera_device_main.h"
...
#include "command/p2p_ipcamera_device_command_common.h"
...
static libpiziot_os_type_func_result_e p2p_ipcamera_device_main_free(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info, int32_t Athread_info_index) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS;
    do {
        p2p_ipcamera_device_main_thread_info_t *lpthread_info = Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_main_thread_info[Athread_info_index];
        ...
        {
            free(lpthread_info);
            Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_main_thread_info[Athread_info_index] = 0;
        }
    } while (0);
    return rval;
}

static libpiziot_os_type_func_result_e p2p_ipcamera_device_main_malloc(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info, int32_t Athread_info_index, uint32_t Asend_data_max_size, uint32_t Arecv_data_max_size) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    p2p_ipcamera_device_main_thread_info_t *lpthread_info = 0;
    do {
        ...
        if (Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_main_thread_info[Athread_info_index] != 0) {
...
#else
        {
            ...
            uint64_t check_size = sizeof(p2p_ipcamera_device_main_thread_info_t);
        ...
        lpthread_info = (p2p_ipcamera_device_main_thread_info_t *)malloc(sizeof(p2p_ipcamera_device_main_thread_info_t));
        if (lpthread_info == 0) {
            TRACEA("%s:out of memory,error !!\n", LIBPIZIOT_OS__FUNCTION__);
            break;
        }
        Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_main_thread_info[Athread_info_index] = lpthread_info;
        {
            memset_addr(p2p_ipcamera_device_main_thread_info_t, lpthread_info, 0, sizeof(p2p_ipcamera_device_main_thread_info_t));
    ...
    if (rval != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
        p2p_ipcamera_device_main_free(Alpp2p_ipcamera_main_device_thread_info, Athread_info_index);
        TRACEA("%s,error !!\n", LIBPIZIOT_OS__FUNCTION__);
    }
    return rval;
}

static void p2p_ipcamera_device_main_start_thread_one(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info, int32_t Athread_info_index, libpiziot_os_pthread_routine_p Alppthread_routine) {
    if (Alppthread_routine == 0) return;
    if (Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_main_thread_info[Athread_info_index] == 0) return;
    {
        p2p_ipcamera_device_main_thread_info_t *lpthread_info = Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_main_thread_info[Athread_info_index];
...
static void p2p_ipcamera_device_main_stop_thread_one(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info, int32_t Athread_info_index) {
    if (Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_main_thread_info[Athread_info_index] == 0) return;
    {
        p2p_ipcamera_device_main_thread_info_t *lpthread_info = Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_main_thread_info[Athread_info_index];
        if (lpthread_info->instance_mutex.init != LIBPIZIOT_OS_MUTEX_STUCT_INIT_CHECK_VALUE) return;
...
void p2p_ipcamera_device_main_stop_thread_all(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info) {
    {
        ...
        for (thread_info_index = 0; thread_info_index < P2P_IPCAMERA_CHANNEL_SERVER_IPCAMERA_DATA_MAIN_THREAD_MAX; thread_info_index++) {
            p2p_ipcamera_device_main_stop_thread_one(Alpp2p_ipcamera_main_device_thread_info, thread_info_index);
        }
    }
    {
        uint32_t thread_info_index;
        for (thread_info_index = 0; thread_info_index < P2P_IPCAMERA_CHANNEL_SERVER_IPCAMERA_DATA_MAIN_THREAD_MAX; thread_info_index++) {
            p2p_ipcamera_device_main_thread_info_t *lpthread_info = Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_main_thread_info[thread_info_index];
    ...
    }
    {
        uint32_t thread_info_index;
        for (thread_info_index = 0; thread_info_index < P2P_IPCAMERA_CHANNEL_SERVER_IPCAMERA_DATA_MAIN_THREAD_MAX; thread_info_index++) {
            if (p2p_ipcamera_device_main_free(Alpp2p_ipcamera_main_device_thread_info, thread_info_index) != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
            }
        }
    }
}

static void p2p_ipcamera_device_main_init_callback(p2p_ipcamera_device_main_thread_info_t *Alpthread_info, libpiziot_p2p_common_channel_data_in_p Alpdata_in_callback) {
...
libpiziot_os_type_func_result_e p2p_ipcamera_device_main_start_thread(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info, int32_t Adevice_handle, int32_t Athread_info_index, int32_t Achannel_id, libpiziot_os_pthread_routine_p Alppthread_routine, libpiziot_p2p_common_channel_data_in_p Alpdata_in_callback, int32_t Asend_data_max_size, int32_t Asend_buffer_size, int32_t Arecv_data_max_size, p2p_func_client_channel_aes_e Aenable_channel_aes) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    do {
        if (p2p_ipcamera_device_main_malloc(Alpp2p_ipcamera_main_device_thread_info, Athread_info_index, Asend_data_max_size, Arecv_data_max_size) != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
            break;
        }
        {
            p2p_ipcamera_device_main_thread_info_t *lpthread_info = Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_main_thread_info[Athread_info_index];
            ...
            }
            {
                ...
                p2p_ipcamera_device_main_init_callback(lpthread_info, Alpdata_in_callback);
                ...
                if (Alppthread_routine != 0) {
                    p2p_ipcamera_device_main_start_thread_one(Alpp2p_ipcamera_main_device_thread_info, Athread_info_index, Alppthread_routine);
                }
            }
        }
        rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS;
...
libpiziot_os_type_func_result_e p2p_ipcamera_device_main_stop_thread(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info, int32_t Athread_info_index) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    do {
        p2p_ipcamera_device_main_stop_thread_one(Alpp2p_ipcamera_main_device_thread_info, Athread_info_index);
        {
            p2p_ipcamera_device_main_thread_info_t *lpthread_info = Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_main_thread_info[Athread_info_index];
            if (lpthread_info != 0) {
                if (lpthread_info->p2p_channel_monitor_handle >= 0) {
        ...
        if (p2p_ipcamera_device_main_free(Alpp2p_ipcamera_main_device_thread_info, Athread_info_index) != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
        }
        rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS;
    } while (0);
    return rval;
...
void p2p_ipcamera_device_main_finalize(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info) {
...
libpiziot_os_type_func_result_e p2p_ipcamera_device_main_initialize(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS;
    do {
        {
            // Initialize channel server thread.
            uint32_t thread_info_index;
            for (thread_info_index = 0; thread_info_index < P2P_IPCAMERA_CHANNEL_SERVER_IPCAMERA_DATA_MAIN_THREAD_MAX; thread_info_index++) {
                Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_main_thread_info[thread_info_index] = 0;
            }
        }
    } while (0);
    if (rval != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
        p2p_ipcamera_device_main_finalize(Alpp2p_ipcamera_main_device_thread_info);
    }
    return rval;
}
