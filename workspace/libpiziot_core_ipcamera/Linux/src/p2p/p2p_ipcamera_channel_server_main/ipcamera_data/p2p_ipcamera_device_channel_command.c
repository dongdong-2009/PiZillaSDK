...
#if defined(P2P_PROTOCOL_IPCAMERA_COMMAND_DEVICE_SEND_DATA_MAX_SIZE)
static struct timeval p2p_ipcamera_device_channel_command_send_packet_timeout = { 30, 0 };
#endif //defined(P2P_PROTOCOL_IPCAMERA_COMMAND_DEVICE_SEND_DATA_MAX_SIZE)

void p2p_ipcamera_device_channel_command_data_in_callback(void *Alpobject, int32_t Adevice_handle, int32_t Achannel_server_handle, int32_t Achannel_id, struct sockaddr_in *Alpin_channel_client_ip, unsigned char *Alpdata, int32_t Adata_size) {
    p2p_ipcamera_device_channel_main_thread_info_t *lpthread_info = Alpobject;
    ...
    do {
        if ((Adata_size > lpthread_info->data_dec_from_client_size) || (Adata_size & 0xF)) {
            TRACEA("PIZCam:data from channel client command size error !\n");
            break;
        }
        {
...
static libpiziot_os_type_func_result_e p2p_ipcamera_device_channel_command_encrypt_data_callback_before_send(unsigned char *Alpdata, int32_t Adata_size, unsigned char *Alpdata_enc, unsigned char **Alppdata_send, void *Alpobject) {
...
static libpiziot_os_type_func_result_e p2p_ipcamera_device_channel_command_send(p2p_ipcamera_device_channel_main_thread_info_t *Alpthread_info, uint32_t Achannel_server_handle, unsigned char *Alpdata_send, int32_t Adata_size, libpiziot_p2p_common_encrypt_data_callback_before_send_p Alpencrypt_data_callback) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    ...
    do {
...
#if defined(LIBPIZIOT_P2P_COMMON_ENABLE_WAN_MODE) || defined(LIBPIZIOT_P2P_COMMON_ENABLE_LAN_MODE)
        {
            ...
            {
                uint32_t identy = Alpthread_info->channel_id + IPCAMERA_DATA_SERVER_SEND_C;
                lpprotocol_head->identy = mem_endian_32(identy);
            }
            lpprotocol_head->sequence = mem_endian_32(Alpthread_info->send_counter);
            rval = libpiziot_p2p_clientsdk_channel_server_send_data(Alpthread_info->p2p_channel_monitor_handle, Achannel_server_handle, Alpdata_send, Adata_size, send_packet_bytes_per_second, p2p_ipcamera_device_channel_command_send_packet_timeout, Alpencrypt_data_callback, Alpthread_info, &(Alpthread_info->net_status));
        }
    ...
    } while (0);
    {
        ...
        if (((int32_t)rval) == LIBPIZIOT_P2P_COMMON_ERR_INVALID_INPUT_LENGTH) {
            TRACEA("PIZCam:DT(%d),ChS(%d),Ch[%d] SEND DATA to cms LIBPIZIOT_P2P_COMMON_ERR_INVALID_INPUT_LENGTH,error.\n", Alpthread_info->thread_index, Alpthread_info->p2p_channel_monitor_handle, Alpthread_info->channel_id);
            usleep(500000);
        }
        else if (rval == LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
            //TRACEA("PIZCam:DC(%d),ChS(%d),Ch[%d] SEND DATA to cms,seq=%d,ok.\n", Alpthread_info->device_handle, Alpthread_info->p2p_channel_monitor_handle, Alpthread_info->channel_id, Alpthread_info->send_counter);
...
libpiziot_os_type_func_result_e p2p_ipcamera_device_channel_command_send_to_channel_client(p2p_ipcamera_device_channel_main_thread_info_t *Alpthread_info, unsigned char *Alpdata_send, int32_t Adata_size) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    if (Alpdata_send == 0 || Adata_size <= 0) return rval;
    if (Adata_size > 0) {
        int32_t enc_size = (Adata_size + 0xF) & 0xFFFFFFF0;
        if (enc_size > p2p_protocol_ipcamera_command_device_send_data_max_size) {
            TRACEA("out of command size(%d) - %d = %d\n", enc_size, p2p_protocol_ipcamera_command_device_send_data_max_size, enc_size - p2p_protocol_ipcamera_command_device_send_data_max_size);
            return rval;
        }
...
libpiziot_os_pthread_dword_t p2p_ipcamera_device_channel_command_thread_instance_routine(void *arg) {
    libpiziot_os_pthread_instance_t *lpthread_instance = (libpiziot_os_pthread_instance_t *)arg;
    p2p_ipcamera_device_channel_main_thread_info_t *lpthread_info = (p2p_ipcamera_device_channel_main_thread_info_t *)(lpthread_instance->m_extra);
...
#if defined(LIBPIZIOT_CORE_P2P_PROTOCOL_IPCAMERA_COMMAND_COMMON)
                        if (p2p_ipcamera_device_channel_command_common_parser(cmd, lpthread_info->thread_index, lpcmd, lphead) == LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) continue;
...
#if defined(LIBPIZIOT_CORE_P2P_PROTOCOL_IPCAMERA_COMMAND_COMMON)
                    p2p_ipcamera_device_channel_command_common_send_action(lpthread_info->thread_index);
#endif //defined(LIBPIZIOT_CORE_P2P_PROTOCOL_IPCAMERA_COMMAND_COMMON)
            } while (0);
                ...
                if (cmd_len > 0) {
                    ...
                    if (lpcmd_info->channel_server_handle != LIBPIZIOT_P2P_COMMON_CHANNEL_SERVER_BROADCAST_DATA) {
                        if (cmd_count > 0) {
                            ...
                            p2p_ipcamera_device_channel_command_send(lpthread_info, LIBPIZIOT_P2P_COMMON_CHANNEL_SERVER_BROADCAST_DATA, (unsigned char *)lpprotocol_head, data_len, p2p_ipcamera_device_channel_command_encrypt_data_callback_before_send);
                            cmd_count = 0;
                        }
                        {
                            int32_t cmd_encrypt_len = (cmd_len + 0xF) & 0xFFFFFFF0;
                            ...
                            cmd_count++;
                            {
                                ...
                                lpprotocol_head->count = mem_endian_32(cmd_count);
                                p2p_ipcamera_device_channel_command_send(lpthread_info, lpcmd_info->channel_server_handle, (unsigned char *)lpprotocol_head, data_len, p2p_ipcamera_device_channel_command_encrypt_data_callback_before_send);
                                cmd_count = 0;
                    ...
                    }
                    {
                        ...
                        cmd_count++;
                        {
                            ...
                            if ((offset + cmd_encrypt_len) >= p2p_protocol_ipcamera_command_device_send_data_max_size) {
                                break;
                            }
                        }
                    }
