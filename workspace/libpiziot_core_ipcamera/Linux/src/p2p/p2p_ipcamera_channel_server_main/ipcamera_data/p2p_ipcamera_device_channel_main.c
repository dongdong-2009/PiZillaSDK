...
#include "command/p2p_ipcamera_device_channel_command_common.h"
...
static libpiziot_p2p_clientsdk_channel_server_callback_t m_p2p_ipcamera_device_callback;

static libpiziot_os_type_func_result_e p2p_ipcamera_device_channel_main_free(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info, int32_t Athread_info_index) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS;
    do {
        p2p_ipcamera_device_channel_main_thread_info_t *lpthread_info = Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_channel_main_thread_info[Athread_info_index];
...
static libpiziot_os_type_func_result_e p2p_ipcamera_device_channel_main_malloc(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info, int32_t Athread_info_index, uint32_t Asend_data_max_size, uint32_t Arecv_data_max_size) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    p2p_ipcamera_device_channel_main_thread_info_t *lpthread_info = 0;
    do {
        ...
        if (Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_channel_main_thread_info[Athread_info_index] != 0) {
...
#else
        {
            ...
            uint64_t check_size = sizeof(p2p_ipcamera_device_channel_main_thread_info_t);
        ...
        Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_channel_main_thread_info[Athread_info_index] = lpthread_info;
        {
            memset_addr(p2p_ipcamera_device_channel_main_thread_info_t, lpthread_info, 0, sizeof(p2p_ipcamera_device_channel_main_thread_info_t));
    ...
    if (rval != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
        p2p_ipcamera_device_channel_main_free(Alpp2p_ipcamera_main_device_thread_info, Athread_info_index);
        TRACEA("%s,error !!\n", LIBPIZIOT_OS__FUNCTION__);
    }
    return rval;
}

static void p2p_ipcamera_device_channel_main_start_thread_one(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info, int32_t Athread_info_index, libpiziot_os_pthread_routine_p Alppthread_routine) {
    if (Alppthread_routine == 0) return;
    if (Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_channel_main_thread_info[Athread_info_index] == 0) return;
    {
        p2p_ipcamera_device_channel_main_thread_info_t *lpthread_info = Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_channel_main_thread_info[Athread_info_index];
        ...
        do {
            ...
            if ((libpiziot_os_pthread_get_status(&(lpthread_info->thread_instance)) == LIBPIZIOT_OS_PTHREAD_STATUS_CLOSED)) {
                if (libpiziot_os_pthread_create(&(lpthread_info->thread_instance), Alppthread_routine, LIBPIZIOT_OS_PTHREAD_STACK_SIZE_AUTO, "core_cam_chs") != 0) {
...
static void p2p_ipcamera_device_channel_main_stop_thread_one(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info, int32_t Athread_info_index) {
    if (Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_channel_main_thread_info[Athread_info_index] == 0) return;
    {
        p2p_ipcamera_device_channel_main_thread_info_t *lpthread_info = Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_channel_main_thread_info[Athread_info_index];
        if (lpthread_info->instance_mutex.init != LIBPIZIOT_OS_MUTEX_STUCT_INIT_CHECK_VALUE) return;
...
void p2p_ipcamera_device_channel_main_stop_thread_all(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info) {
    {
        uint32_t thread_info_index;
        for (thread_info_index = 0; thread_info_index < P2P_IPCAMERA_CHANNEL_SERVER_IPCAMERA_DATA_MAIN_THREAD_MAX; thread_info_index++) {
            p2p_ipcamera_device_channel_main_stop_thread_one(Alpp2p_ipcamera_main_device_thread_info, thread_info_index);
        }
    }
    {
        uint32_t thread_info_index;
        for (thread_info_index = 0; thread_info_index < P2P_IPCAMERA_CHANNEL_SERVER_IPCAMERA_DATA_MAIN_THREAD_MAX; thread_info_index++) {
            p2p_ipcamera_device_channel_main_thread_info_t *lpthread_info = Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_channel_main_thread_info[thread_info_index];
    ...
    }
    {
        uint32_t thread_info_index;
        for (thread_info_index = 0; thread_info_index < P2P_IPCAMERA_CHANNEL_SERVER_IPCAMERA_DATA_MAIN_THREAD_MAX; thread_info_index++) {
            if (p2p_ipcamera_device_channel_main_free(Alpp2p_ipcamera_main_device_thread_info, thread_info_index) != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
            }
        }
    }
}

static void p2p_ipcamera_device_channel_main_init_callback(p2p_ipcamera_device_channel_main_thread_info_t *Alpthread_info, libpiziot_p2p_common_channel_data_in_p Alpdata_in_callback) {
    memset_addr(libpiziot_p2p_clientsdk_channel_server_callback_t, &m_p2p_ipcamera_device_callback, 0, sizeof(m_p2p_ipcamera_device_callback));
    {
        m_p2p_ipcamera_device_callback.lpobject = Alpthread_info;
        m_p2p_ipcamera_device_callback.break_callback.lpthread_instance = &(Alpthread_info->thread_instance);
        m_p2p_ipcamera_device_callback.lpdata_in_callback = Alpdata_in_callback;
    }
}

libpiziot_os_type_func_result_e p2p_ipcamera_device_channel_main_start_thread(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info, int32_t Adevice_handle, int32_t Athread_info_index, int32_t Achannel_id, libpiziot_os_pthread_routine_p Alppthread_routine, libpiziot_p2p_common_channel_data_in_p Alpdata_in_callback, int32_t Asend_data_max_size, int32_t Asend_buffer_size, int32_t Arecv_data_max_size, p2p_func_client_channel_aes_e Aenable_channel_aes) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    do {
        if (p2p_ipcamera_device_channel_main_malloc(Alpp2p_ipcamera_main_device_thread_info, Athread_info_index, Asend_data_max_size, Arecv_data_max_size) != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
            break;
        }
        {
            p2p_ipcamera_device_channel_main_thread_info_t *lpthread_info = Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_channel_main_thread_info[Athread_info_index];
            ...
            }
            {
                ...
                p2p_ipcamera_device_channel_main_init_callback(lpthread_info, Alpdata_in_callback);
                lpthread_info->thread_index = Athread_info_index;
                {
                    ...
                    if (p2p_func_client_device_get_ipcamera_channel_key(&(lpthread_info->login_aes.aes_key), lpthread_info->login_aes.cbc_iv, channel_account, LIBPIZIOT_P2P_COMMON_CHANNEL_ACCOUNT_LENGTH_MAX, channel_password, LIBPIZIOT_P2P_COMMON_CHANNEL_PASSWORD_LENGTH_MAX) != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) break;
                    // Call the P2P SDK to create a new channel server socket.
                    {
                        ...
                        if (libpiziot_p2p_clientsdk_listen_channel_server(Adevice_handle, Achannel_id, channel_account, channel_password, &m_p2p_ipcamera_device_callback, send_data_max_size, send_buffer_size, recv_data_max_size, &(lpthread_info->p2p_channel_monitor_handle)) != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
                            TRACEA("PIZCam:create channel server,error !!\n");
                            break;
                        }
                    }
                }
                if (Alppthread_routine != 0) {
                    p2p_ipcamera_device_channel_main_start_thread_one(Alpp2p_ipcamera_main_device_thread_info, Athread_info_index, Alppthread_routine);
                }
            }
        }
        rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS;
...
libpiziot_os_type_func_result_e p2p_ipcamera_device_channel_main_stop_thread(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info, int32_t Athread_info_index) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    do {
        p2p_ipcamera_device_channel_main_stop_thread_one(Alpp2p_ipcamera_main_device_thread_info, Athread_info_index);
        {
            p2p_ipcamera_device_channel_main_thread_info_t *lpthread_info = Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_channel_main_thread_info[Athread_info_index];
            if (lpthread_info != 0) {
                if (lpthread_info->p2p_channel_monitor_handle >= 0) {
        ...
        if (p2p_ipcamera_device_channel_main_free(Alpp2p_ipcamera_main_device_thread_info, Athread_info_index) != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
        }
        rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS;
    } while (0);
    return rval;
...
void p2p_ipcamera_device_channel_main_finalize(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info) {
    if (Alpp2p_ipcamera_main_device_thread_info) {
        LIBPIZIOT_FIX_ANDROID_COMPILE_MIPS_ERROR(0);
    }
}

// Initialize channel server thread.
libpiziot_os_type_func_result_e p2p_ipcamera_device_channel_main_initialize(p2p_ipcamera_main_device_thread_info_t *Alpp2p_ipcamera_main_device_thread_info) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS;
    do {
        {
            // Initialize channel server thread.
            uint32_t thread_info_index;
            for (thread_info_index = 0; thread_info_index < P2P_IPCAMERA_CHANNEL_SERVER_IPCAMERA_DATA_MAIN_THREAD_MAX; thread_info_index++) {
                Alpp2p_ipcamera_main_device_thread_info->lpp2p_ipcamera_device_channel_main_thread_info[thread_info_index] = 0;
            }
        }
    } while (0);
    if (rval != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
        p2p_ipcamera_device_channel_main_finalize(Alpp2p_ipcamera_main_device_thread_info);
    }
    return rval;
}
