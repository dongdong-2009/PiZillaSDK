...
#include "../p2p_ipcamera_channel_server_main/ipcamera_data/command/p2p_ipcamera_device_channel_command_common.h"
...
typedef enum p2p_ipcamera_main_device_thread_step_enum {
    P2P_IPCAMERA_MAIN_DEVICE_THREAD_STEP_NORMAL = 1,
#if defined(SAMPLE_CODE_ENABLE_DEVICE_EXIST_CHECKER)
    P2P_IPCAMERA_MAIN_DEVICE_THREAD_STEP_CHECKER = 2,
#endif //defined(SAMPLE_CODE_ENABLE_DEVICE_EXIST_CHECKER)
} p2p_ipcamera_main_device_thread_step_e;

#if defined(SAMPLE_CODE_ENABLE_DEVICE_EXIST_CHECKER)
static int32_t p2p_ipcamera_main_device_exist_checker_handle = -1;
#endif //defined(SAMPLE_CODE_ENABLE_DEVICE_EXIST_CHECKER)

char *libpiziot_core_p2p_get_ipcamera_uid(void) {
#if defined(ENABLE_P2P_IPCAMERA) && (defined(ENABLE_P2P_CMS) || defined(ENABLE_P2P_NAS))
    const char *lpUID = p2p_func_client_get_ipcamera_uuid_from_mtd();
...
static libpiziot_os_type_func_result_e p2p_ipcamera_main_device_create_p2p_device_exist_checker(const char *AlpUID) {
...
#define CONNECT_SERVER_SOCKET_INFO_COUNT 1
    ...
    {
        TRACEA("PIZCam:Check UID=\"%s\"\n", AlpUID);
    }
    rval = libpiziot_p2p_clientsdk_create_viewer(connect_server_socket_info, CONNECT_SERVER_SOCKET_INFO_COUNT, SAMPLE_CODE_P2P_LAN_SERVER_PORT_DEFAULT, AlpUID, select_net_type, (unsigned char *) "", (unsigned char *) "", &p2p_ipcamera_main_device_exist_checker_handle);
    if (rval != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
        TRACEA("PIZCam:Check UID=\"%s\",error !!\n", AlpUID);
    }
    return rval;
}

static void p2p_ipcamera_main_device_delete_p2p_device_exist_checker(void) {
    if (p2p_ipcamera_main_device_exist_checker_handle >= 0) {
        libpiziot_p2p_clientsdk_delete_viewer(p2p_ipcamera_main_device_exist_checker_handle);
        p2p_ipcamera_main_device_exist_checker_handle = -1;
    }
}

#endif //defined(SAMPLE_CODE_ENABLE_DEVICE_EXIST_CHECKER)

static void p2p_ipcamera_main_device_channel_update_mode(p2p_ipcamera_main_device_thread_info_t *Alpthread_info, int32_t Adevice_handle, int32_t Achannel_id, libpiziot_os_pthread_routine_p Alppthread_routine, libpiziot_p2p_common_channel_data_in_p Alpdata_in_callback, int32_t Asend_data_max_size, int32_t Asend_buffer_size, int32_t Arecv_data_max_size, p2p_func_client_channel_aes_e Aenable_channel_aes, p2p_ipcamera_device_channel_main_protocol_command_t *Alpprotocol_command) {
    ...
    if (mode == P2P_FUNC_CLIENT_CHANNEL_ENABLE_YES) {
        p2p_ipcamera_device_channel_main_start_thread(Alpthread_info, Adevice_handle, Alpthread_info->update_thread_info_index, Achannel_id, Alppthread_routine, Alpdata_in_callback, Asend_data_max_size, Asend_buffer_size, Arecv_data_max_size, Aenable_channel_aes);
        {
            libpiziot_os_mutex_plock_lock(&p2p_ipcamera_main_device_channel_mutex);
            {
                Alpprotocol_command->lpthread_info = Alpthread_info->lpp2p_ipcamera_device_channel_main_thread_info[Alpthread_info->update_thread_info_index];
#if defined(LIBPIZIOT_CORE_P2P_PROTOCOL_IPCAMERA_COMMAND)
#if defined(LIBPIZIOT_CORE_P2P_PROTOCOL_IPCAMERA_COMMAND_COMMON)
                {
                    p2p_ipcamera_device_channel_command_common_init_action(Alpprotocol_command);
                }
#endif //defined(LIBPIZIOT_CORE_P2P_PROTOCOL_IPCAMERA_COMMAND_COMMON)
#endif //defined(LIBPIZIOT_CORE_P2P_PROTOCOL_IPCAMERA_COMMAND)
            }
    ...
    if (mode == P2P_FUNC_CLIENT_CHANNEL_ENABLE_NO) {
        ...
        libpiziot_os_mutex_plock_unlock(&p2p_ipcamera_main_device_channel_mutex);
        p2p_ipcamera_device_channel_main_stop_thread(Alpthread_info, Alpthread_info->update_thread_info_index);
    }
    Alpthread_info->update_thread_info_index++;
}

static void p2p_ipcamera_main_device_start_channel_thread(p2p_ipcamera_main_device_thread_info_t *Alpthread_info) {
    if (Alpthread_info) {
        Alpthread_info->update_thread_info_index = 0;
#if defined(LIBPIZIOT_CORE_P2P_PROTOCOL_IPCAMERA_COMMAND)
        p2p_ipcamera_main_device_channel_update_mode(Alpthread_info, Alpthread_info->p2p_ipcamera_main_device_handle, LIBPIZIOT_CORE_P2P_PROTOCOL_IPCAMERA_COMMAND, p2p_ipcamera_device_channel_command_thread_instance_routine, p2p_ipcamera_device_channel_command_data_in_callback, p2p_protocol_ipcamera_command_device_send_data_max_size, p2p_protocol_ipcamera_command_device_send_data_max_size, p2p_protocol_ipcamera_command_device_recv_data_max_size, P2P_FUNC_CLIENT_CHANNEL_AES_ENABLE, &(Alpthread_info->protocol_command));
#endif //defined(LIBPIZIOT_CORE_P2P_PROTOCOL_IPCAMERA_COMMAND)
        ...
        p2p_ipcamera_main_device_channel_update_mode(Alpthread_info, Alpthread_info->p2p_ipcamera_main_device_handle, LIBPIZIOT_CORE_P2P_PROTOCOL_IPCAMERA_OTHER, p2p_ipcamera_device_channel_other_thread_instance_routine, p2p_ipcamera_device_channel_other_data_in_callback, p2p_protocol_ipcamera_other_device_send_data_max_size, p2p_protocol_ipcamera_other_device_send_data_max_size * 2, p2p_protocol_ipcamera_other_device_recv_data_max_size, P2P_FUNC_CLIENT_CHANNEL_AES_ENABLE, &(Alpthread_info->protocol_other));
#endif //defined(LIBPIZIOT_CORE_P2P_PROTOCOL_IPCAMERA_OTHER)
    }
}

static void p2p_ipcamera_main_device_stop_channel_thread(p2p_ipcamera_main_device_thread_info_t *Alpthread_info) {
    p2p_ipcamera_device_channel_main_stop_thread_all(Alpthread_info);
}

static void p2p_ipcamera_main_device_create_p2p_ipcamera(p2p_ipcamera_main_device_thread_info_t *Alpthread_info, const char *AlpUID) {
...
#define CONNECT_SERVER_SOCKET_INFO_COUNT 1
        ...
        if (p2p_func_client_device_get_ipcamera_device_key(&(Alpthread_info->login_aes.aes_key), Alpthread_info->login_aes.cbc_iv, device_account, LIBPIZIOT_P2P_COMMON_DEVICE_ACCOUNT_LENGTH_MAX, device_password, LIBPIZIOT_P2P_COMMON_DEVICE_PASSWORD_LENGTH_MAX) != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) return;

        if (libpiziot_p2p_clientsdk_create_device(connect_server_socket_info, CONNECT_SERVER_SOCKET_INFO_COUNT, device_port, AlpUID, select_net_type, device_account, device_password, &(Alpthread_info->p2p_ipcamera_main_device_handle)) == LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
        }
    }
}

static void p2p_ipcamera_main_device_delete_p2p_device(p2p_ipcamera_main_device_thread_info_t *Alpthread_info) {
    p2p_ipcamera_main_device_stop_channel_thread(Alpthread_info);
    if (Alpthread_info->p2p_ipcamera_main_device_handle >= 0) {
        libpiziot_p2p_clientsdk_delete_device(Alpthread_info->p2p_ipcamera_main_device_handle);
        Alpthread_info->p2p_ipcamera_main_device_handle = -1;
    }
}

static libpiziot_os_pthread_dword_t p2p_ipcamera_main_device_thread_instance_routine(void *arg) {
    libpiziot_os_pthread_instance_t *lpthread_instance = (libpiziot_os_pthread_instance_t *)arg;
    p2p_ipcamera_main_device_thread_info_t *lpthread_info = (p2p_ipcamera_main_device_thread_info_t *)(lpthread_instance->m_extra);
#if defined(SAMPLE_CODE_ENABLE_DEVICE_EXIST_CHECKER)
    libpiziot_p2p_common_result_e viewer_net_result_last = LIBPIZIOT_P2P_COMMON_RESULT_UNKNOWN_MESSAGE_TYPE;
    p2p_ipcamera_main_device_thread_step_e thread_step = P2P_IPCAMERA_MAIN_DEVICE_THREAD_STEP_CHECKER;
#else
    p2p_ipcamera_main_device_thread_step_e thread_step = P2P_IPCAMERA_MAIN_DEVICE_THREAD_STEP_NORMAL;
#endif //defined(SAMPLE_CODE_ENABLE_DEVICE_EXIST_CHECKER)
    ...
    do {
        ...
        switch (thread_step) {
        case P2P_IPCAMERA_MAIN_DEVICE_THREAD_STEP_NORMAL: {
            if (lpthread_info->p2p_ipcamera_main_device_handle == -1) {
                // Call the P2P SDK to create device with UID.
                p2p_ipcamera_main_device_create_p2p_ipcamera(lpthread_info, lpthread_info->lpuuid);
                continue;
            }
            else {
                p2p_ipcamera_main_device_start_channel_thread(lpthread_info);
            }
        }
                                                          break;
#if defined(SAMPLE_CODE_ENABLE_DEVICE_EXIST_CHECKER)
        case P2P_IPCAMERA_MAIN_DEVICE_THREAD_STEP_CHECKER: {
            if (p2p_ipcamera_main_device_exist_checker_handle == -1) {
                // Call the P2P SDK to create a viewer to check if duplicate UID device exist.
                if (p2p_ipcamera_main_device_create_p2p_device_exist_checker(lpthread_info->lpuuid) != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
            ...
            //Check if duplicate UID device exist after viewer create.
            {
                ...
                libpiziot_os_type_func_result_e rval = libpiziot_p2p_clientsdk_get_viewer_net_status(p2p_ipcamera_main_device_exist_checker_handle, &viewer_net_result);
                //OK ?
                if (rval == LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
                    ...
                    if (viewer_net_result != viewer_net_result_last) {
                        ...
                        case LIBPIZIOT_P2P_COMMON_RESULT_MODE_WAN_DEVICE: {
                            ...
                            p2p_ipcamera_main_device_delete_p2p_device_exist_checker();
                        ...
                        default: {
                            ...
                            if (lpthread_info->p2p_ipcamera_main_device_handle == -1) {
                                ...
                                thread_step = P2P_IPCAMERA_MAIN_DEVICE_THREAD_STEP_NORMAL;
                            }
                        }
                                 break;
                        }
    ...
    p2p_ipcamera_main_device_delete_p2p_device(lpthread_info);
...
libpiziot_os_type_func_result_e p2p_ipcamera_main_device_listen(p2p_ipcamera_main_device_thread_info_t *Alpthread_info, const char *AlpUID) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    if (Alpthread_info == 0) return rval;
    {
        ...
        do {
            ...
            if ((libpiziot_os_pthread_get_status(&(Alpthread_info->thread_instance)) == LIBPIZIOT_OS_PTHREAD_STATUS_CLOSED)) {
                ...
                if (libpiziot_os_pthread_create(&(Alpthread_info->thread_instance), p2p_ipcamera_main_device_thread_instance_routine, LIBPIZIOT_OS_PTHREAD_STACK_SIZE_AUTO, "core_cam") != 0) {
...
void p2p_ipcamera_main_device_unlisten(p2p_ipcamera_main_device_thread_info_t *Alpthread_info) {
...
libpiziot_os_type_func_result_e p2p_ipcamera_main_device_finalize(p2p_ipcamera_main_device_thread_info_t **Alpp_thread_info) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS;
    do {
        p2p_ipcamera_main_device_thread_info_t *lpthread_info = (*Alpp_thread_info);
        ...
        if (lpthread_info->instance_mutex.init != LIBPIZIOT_OS_MUTEX_STUCT_INIT_CHECK_VALUE) break;
        {
            ...
            p2p_ipcamera_device_channel_main_finalize(lpthread_info);
        }
        {
...
libpiziot_os_type_func_result_e p2p_ipcamera_main_device_initialize(p2p_ipcamera_main_device_thread_info_t **Alpp_thread_info) {
    libpiziot_os_type_func_result_e rval = LIBPIZIOT_OS_TYPE_FUNC_RESULT_FAILURE;
    p2p_ipcamera_main_device_thread_info_t *lpthread_info = 0;
    do {
...
#else
        {
            ...
            if ((free_memory_size + sizeof(p2p_ipcamera_main_device_thread_info_t)) < LIBPIZIOT_P2P_COMMON_SYSTEM_MEMORY_MINIMUM_SIZE) {
        ...
        lpthread_info = (p2p_ipcamera_main_device_thread_info_t *)libpiziot_os_malloc(sizeof(p2p_ipcamera_main_device_thread_info_t));
        if (lpthread_info == 0) {
        ...
        {
            memset_addr(p2p_ipcamera_main_device_thread_info_t, lpthread_info, 0, sizeof(p2p_ipcamera_main_device_thread_info_t));
            lpthread_info->p2p_ipcamera_main_device_handle = -1;
#if defined(LIBPIZIOT_CORE_P2P_PROTOCOL_IPCAMERA_COMMAND)
            ...
            lpthread_info->thread_instance.m_extra = lpthread_info;
            {
                ...
                {
                    if (p2p_ipcamera_device_channel_main_initialize(lpthread_info) != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
                        break;
                    }
                }
            }
    ...
    if (rval != LIBPIZIOT_OS_TYPE_FUNC_RESULT_SUCCESS) {
        p2p_ipcamera_main_device_finalize(Alpp_thread_info);
        TRACEA("%s,error !!\n", LIBPIZIOT_OS__FUNCTION__);
    }
    return rval;
}
